...
export const resolveMarketplaceConfig = (): IMarketplaceConfig => {
  ...
  if (pkgJSON && pkgJSON[EXTENSION_CONFIG_FIELD]) {
    ...
    if (config.endpoint) {
      log.info(
        '使用 package.json 中的定义的 marketplaceConfig.endpoint 配置: ' + pkgJSON[EXTENSION_CONFIG_FIELD].endpoint,
      );
      MARKETPLACE_CONFIG.endpoint = config.endpoint;
    }
    if (config.accountId) {
      log.info(
        '使用 package.json 中的定义的 marketplaceConfig.accountId 配置: ' + pkgJSON[EXTENSION_CONFIG_FIELD].accountId,
      );
